...
async function apiFetch(path, opts) {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    const xv = (function () { try { return chrome.runtime.getManifest().version; } catch (e) { return ""; } })();
    const headers = Object.assign({}, opts.headers, sig, { "x-roo-version": xv }, h ? { "x-app-key": h } : {}, tok ? { "x-roo-session": tok } : {});
    let retry = false;
...
function myVersion() { try { return chrome.runtime.getManifest().version; } catch (e) { return ""; } }
// Hard "Update required" screen — the extension is unusable on the wrong version.
function showUpdate(required, current) {
  try { lockScreen.classList.add("hidden"); mainScreen.classList.add("hidden"); } catch (e) {}
  let d = document.getElementById("updateScreen");
  if (!d) { d = document.createElement("div"); d.id = "updateScreen"; document.body.appendChild(d); }
  d.style.cssText = "padding:20px 16px;text-align:center;color:#f1e9ff;";
  d.innerHTML =
    '<h2 style="margin:6px 0 8px;font-size:17px;font-weight:700;">Update required</h2>' +
    '<p style="font-size:12px;opacity:.85;line-height:1.55;">You\'re on <b>v' + (current || "?") + '</b> — the latest is <b>v' + (required || "?") + '</b>. You must update to keep using the extension. Your licence is <b>paused</b> (no time lost) until any of your devices updates.</p>' +
    '<ol style="text-align:left;font-size:12px;line-height:1.7;padding-left:20px;margin:14px 0;">' +
    '<li>Open the <b>Extensions</b> tab (chrome://extensions)</li>' +
    '<li>Click <b>Remove</b> on the old version (v' + (current || "?") + ')</li>' +
    '<li>Click <b>Load unpacked</b> and select the new version folder</li>' +
    '<li>Reopen the extension</li></ol>' +
    '<a href="https://github.com/VisualTFX/deliveroo-ext/releases/" target="_blank" style="display:block;text-decoration:none;text-align:center;padding:12px;border-radius:11px;font-size:14px;font-weight:700;color:#fff;background:linear-gradient(135deg,#8600ff,#b478ff);box-shadow:0 8px 22px rgba(134,0,255,.4);">Get the latest version →</a>';
}

// Shown while the in-tab key-activation sequence is running — so the user can't use the popup mid-activation.
function showActivating() {
  try { lockScreen.classList.add("hidden"); mainScreen.classList.add("hidden"); } catch (e) {}
  try { hideDevicePicker(); hideIpReset(); } catch (e) {}
  let d = document.getElementById("activatingScreen");
  if (!d) { d = document.createElement("div"); d.id = "activatingScreen"; document.body.appendChild(d); }
  d.style.cssText = "padding:34px 18px;text-align:center;color:#f1e9ff;";
  d.innerHTML =
    '<div style="width:46px;height:46px;margin:6px auto 16px;border-radius:50%;border:4px solid rgba(180,120,255,.18);border-top-color:#b478ff;border-right-color:#b478ff;animation:ipspin .8s linear infinite;"></div>' +
    '<h2 style="margin:0 0 6px;font-size:16px;font-weight:700;">Activating key…</h2>' +
    '<p style="font-size:12px;opacity:.82;line-height:1.55;">Setting up and clearing your Deliveroo session on the open tab. This takes a few seconds — please wait.</p>';
}
function hideActivating() { const d = document.getElementById("activatingScreen"); if (d) d.remove(); }
...
chrome.storage.local.get(["license", "__outdated", "__requiredVersion", "__activating"], (d) => {
  if (d && d.__activating && (Date.now() - d.__activating < 15000)) { showActivating(); return; } // mid-activation → block the popup
  if (d && d.__outdated) { showUpdate(d.__requiredVersion, myVersion()); revalidate((d.license || {}).key); return; } // re-check in case they updated
...
// React live if the activation sequence starts/ends while the popup is open.
chrome.storage.onChanged.addListener((ch, area) => {
  if (area !== "local" || !ch.__activating) return;
  if (ch.__activating.newValue) { showActivating(); return; }
  hideActivating(); // activation finished → restore the right screen
  chrome.storage.local.get(["license", "__outdated", "__requiredVersion"], (d) => {
    if (d && d.__outdated) { showUpdate(d.__requiredVersion, myVersion()); return; }
    const lic = d && d.license;
    if (licValid(lic)) showMain(lic); else showLock("");
  });
});
...
async function revalidate(key) {
  try {
    ...
    // version gate: wrong extension version → hard-block with the update screen
    if (typeof data.version_ok === "boolean") {
      await chrome.storage.local.set({ __outdated: !data.version_ok, __requiredVersion: data.required_version || "" });
      if (!data.version_ok) { showUpdate(data.required_version, myVersion()); return; }
      else { const u = document.getElementById("updateScreen"); if (u) u.remove(); }
    }
    if (!data.valid) {
...
async function doUnlock() {
  ...
  try {
    ...
    if (data.valid && data.expires_at) {
      ...
      applySmsSync(data);
      // user-initiated activation (any key, even the same one) → run the activation sequence
      try { chrome.runtime.sendMessage({ type: "rooKeyActivated" }, () => void chrome.runtime.lastError); } catch (e) {}
...
function initMain() {
  ...
  function showResetView(fromIp) {
    ...
    const desktop = isDesktopUA(); // desktop → VPN / hotspot guide; mobile → airplane-mode guide
